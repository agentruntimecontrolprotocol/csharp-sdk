...
await Request(client, Envelope(client, ExtTune, new ExtensionPayload(...)));
await Request(client, Envelope(client, ExtGain, new ExtensionPayload(...)));
Env capture = await Request(client, Envelope(client, ExtCapture, new ExtensionPayload(...)));
Env audio   = await Request(client, Envelope(client, ExtDemodulate,
    new ExtensionPayload(JsonSerializer.SerializeToElement(new
    {
...
- `Program.cs` — client side: tune, gain, capture, demodulate, then
  one optional unknown for §21.3.
- `ExtensionPayload.cs` — generic JsonElement payload record.
