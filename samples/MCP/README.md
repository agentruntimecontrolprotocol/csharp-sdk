...
which doesn't compose with the rest of your stack.
...
MCP tool refuse the session if it's not advertised.
...
`CallViaMcpAsync` translates MCP errors into canonical ARCP error
...
- `Program.cs` — the bridge. `HandleInvokeAsync` is the file.
  Runtime wiring (transport, session manager) is symmetric with
  `ARCP.Runtime.ARCPRuntime` and elided.
- `Upstream.cs` — MCP server invocation + tiny stub for the
  `ModelContextProtocol` NuGet package types we depend on.
