...
separate per-provider sidecar dashboard. The router has no idea
which provider 429s right now, so retries spray across already-saturated
...
List<string> chain = CandidateChain(profiles, requestClass: "balanced");
Env reply = await InvokeWithFallbackAsync(clients, chain,
    tool: "chat.completion", arguments: new { prompt = "Hello" }, traceId);
```
...
- `Program.cs` — open all peers, route one request, print rollup.
- `Profile.cs` — typed view of marketplace capability fields.
