...
Thanks for your interest in improving the C# SDK for ARCP. This
...
- **This SDK** — how the protocol is expressed idiomatically in C#:
...
.NET 10 SDK from [dotnet.microsoft.com/download](https://dotnet.microsoft.com/download), then clone and
...
- **Conformance** — the SDK's behavior against the reference runtime. New
  ...
  assumes the answer. The conformance suite in
  `tests/Arcp.ConformanceTests/` pairs `Arcp.Client` with `Arcp.Runtime` over
  `MemoryTransport` so every fact runs against the in-tree reference
  implementation; run it on its own with
  `dotnet test tests/Arcp.ConformanceTests/Arcp.ConformanceTests.csproj`, and
  see [`CONFORMANCE.md`](./CONFORMANCE.md) for the spec-section-to-test map.
